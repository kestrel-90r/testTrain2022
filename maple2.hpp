...
struct Chunk
{
	...
	Mat4x4 mat()
	{
		//chunkで最終空間位置に移動(ユーザーTRS+モーションTRS)
...
struct Entity
{
	std::string meshtag;	//meshTAG、略称。glTFのmeshのname終端に対応 Station.A1
	int32 meshid;			//meshID、glTFのmeshのmesh番号に対応

	Float3 pos{ 0,0,0 };	//3Dグリッド計算済の位置
	Float3 scale{ 1,1,1 };	//倍率
	Quaternion qrotate = Quaternion::Identity();//回転
};
...
struct Maple
{
	std::string grid = {};		// 2D文字グリッド
	Size size = {};				// グリッドサイズ
	Array<uint16> refgrid;		// 2D値グリッド(displaylist参照インデクスのグリッド)
	Array<Entity> displaylist;	// 3D描画リスト

	Maple() = default;
	Maple(const std::string& map) : grid{ map }
	{
		size = GetSize(grid);	//サイズ取得
	...
	//               取得座標XY    検索開始位置  文字マップ      検索タグ      文字マップ幅
	static size_t GetPoint(Point& point, size_t start, std::string& mapstr, std::string& tag, uint32 width)
	{
		...
		if (index == std::string::npos) return std::string::npos;	//ない
		point.x = index % width;
		point.y = index / width;
		return index;
	}
...
void initMaple(Maple& maple, Array<Entity>& entities, Size unit = Size{ 4,2 })
{
	...
	if (maple.displaylist.size() == 0)
	{
		...
		for (auto& ent : entities)	//登録されている物体順にchunkローカル座標取得
		{
			size_t index = 0;
			for (;;)
			{
				...
				ent.pos = Float3{ point.x / unit.x, 0, point.y / unit.y };	//XZ平面化
				maple.displaylist.emplace_back(ent);		//ディスプレイリスト(meshID)
				index++;
			}
		}
